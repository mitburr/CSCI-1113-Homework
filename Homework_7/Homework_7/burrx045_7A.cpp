...
//  main.cpp
//
//  Created by Mitchell Burr on 11/13/18.
...
public:
    ...
    bool isDead(); // checks to see if you have enough hp
    int getHP();
    int getGOLD();
    int getDMG();
    ...
    int getRowLocation();
    int getColLocation();
    void loot(creature& victim);
    void GetWeapon(int damage);
    void BuyWeapon(int cost);
...
public:
    ...
    creature& getCreature(string name); // ^^ (probably better to use pointers, but this isn't about them)
...
public:
    weapon();
    weapon(string name, int damage, int gold);
    int getWeaponGold();
    int getWeaponDmg();
...
int main()
{
    ...
    while(!island.getCreature(hname).isDead()) // keep going until you drop
    {
        ...
        // show gold count
        cout << "Gold: " << island.getCreature(hname).getGOLD() << endl;
...
void gotoTown(creature& you)
{
    ...
    cout << you.getName() << " walks into town. Do you want to buy a sharp pointy (b)oi for 13gp or leave (any other key)?";
    string b;
    getline(cin, b);
    if (b == "b"){
        you.BuyWeapon()
        you.GetWeapon(<#int damage#>)();
    }
    else
    {
        cout << "go get that bread fool" << endl;
        cout << "you leave town" << endl;
    }
...
void creature::loot(creature& victim)
{
    gold += victim.gold;
}

void creature::GetWeapon(int damage)
{
    dmg += damage;
    cout << "Don't trip now..." << endl;
}

void creature::BuyWeapon(int cost)
{
    gold -= cost;
}
...
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// -=-=-=- Weapons class stuff below -=-
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

weapon::weapon(string name, int damage, int gold)
{
    WeaponDmg = damage;
    WeaponGold = gold;
    WeaponName = name;
...
int weapon::getWeaponGold(){return WeaponGold;}
int weapon::getWeaponDmg(){return WeaponDmg;}
